...
function getWorkstreamPath(versionTag, workstreamId, cwd = process.cwd(), versionsDir = "versions") {
  ...
  const workstreamsDir = path.join(implementPath, "workstreams");
  if (!fs.existsSync(workstreamsDir)) return null;
  
  // First try exact match
  let workstreamPath = path.join(workstreamsDir, workstreamId);
  if (fs.existsSync(workstreamPath)) {
    return workstreamPath;
  }
  
  // If not found, try to find by matching metadata (in case workstreamId is original name but folder is sanitized)
  try {
    const entries = fs.readdirSync(workstreamsDir, { withFileTypes: true });
    for (const entry of entries) {
      if (entry.isDirectory()) {
        const metadataPath = path.join(workstreamsDir, entry.name, "workstream.json");
        if (fs.existsSync(metadataPath)) {
          try {
            const metadata = JSON.parse(fs.readFileSync(metadataPath, "utf8"));
            // Check if workstreamId matches id, name, or sanitizedName
            if (metadata.id === workstreamId || 
                metadata.name === workstreamId || 
                metadata.sanitizedName === workstreamId ||
                entry.name === workstreamId) {
              return path.join(workstreamsDir, entry.name);
            }
          } catch (e) {
