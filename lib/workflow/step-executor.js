    ...
    async executeStep(executionId, step, state, workflow) {
      ...
      try {
        ...
        const context = {
          ...
          stepId: step.id,
          state,
          engine, // Pass engine for database access
          db: engine.db
        };
        ...
        if (outputs && outputs.status === "pending" && outputs.decisionId) {
          // Mark execution as waiting for decision
          ...
          engine.db.addHistory(executionId, "step_pending_decision", step.id, {
            decisionId: outputs.decisionId,
