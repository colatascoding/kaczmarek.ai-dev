...
function createWorkflowExecutor(engine) {
  return {
    ...
    async executeWorkflow(executionId, workflow, triggerData, versionTag = null) {
      ...
      let stepCount = 0;
      const visitedSteps = new Set(); // Track visited steps to detect cycles
      
      while (currentStep && stepCount < MAX_STEPS) {
        stepCount++;
        
        // Detect cycles - if we've visited this step more than once, break
        if (visitedSteps.has(currentStep.id)) {
          engine.db.updateExecution(executionId, {
            status: "failed",
            error: `Workflow cycle detected: step ${currentStep.id} was visited multiple times`
          });
          engine.db.addHistory(executionId, "workflow_failed", null, { 
            error: "Cycle detected",
            stepId: currentStep.id
          });
          throw new Error(`Workflow cycle detected: step ${currentStep.id} was visited multiple times`);
        }
        ...
        // Determine next step
        const nextStep = engine.utils.determineNextStep(
          currentStep,
          result,
          workflow,
        ...
        // If next step is the same as current, break to prevent infinite loop
        if (nextStep && nextStep.id === currentStep.id) {
          engine.db.updateExecution(executionId, {
            status: "failed",
            error: `Workflow loop detected: step ${currentStep.id} points to itself`
          });
          engine.db.addHistory(executionId, "workflow_failed", null, { 
      ...
      if (stepCount >= MAX_STEPS) {
        engine.db.updateExecution(executionId, {
          status: "failed",
          error: `Workflow exceeded maximum step limit (${MAX_STEPS})`
        });
        engine.db.addHistory(executionId, "workflow_failed", null, { 
    ...
    async resumeExecution(executionId, workflow, state, decisionData) {
      ...
      // Priority: decisionData.stepId > execution.currentStep > state.workflow.waitingStepId
      const execution = engine.db.getExecution(executionId);
      const waitingStepId = decisionData.stepId || execution?.currentStep || state.workflow?.waitingStepId;
      
      if (!waitingStepId) {
        throw new Error("Cannot determine waiting step ID - no stepId in decision data, execution, or state");
      }
