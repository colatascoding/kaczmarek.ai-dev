...
class WorkflowEngine {
  ...
  async executeWorkflow(executionId, workflow, triggerData, versionTag = null) {
    ...
    const summary = this.generateExecutionSummary(executionId, workflow, state, outcome, followUpSuggestions);
  ...
  async executeNextStep(executionId, workflow, triggerData, versionTag = null) {
    ...
    if (!execution.currentStep) {
      ...
      currentStep = this.determineNextStep(lastStep, lastResult, workflow);
    }
    ...
    this.db.addHistory(executionId, "step_completed", currentStep.id);
  ...
  generateExecutionSummary(executionId, workflow, state, outcome, followUpSuggestions) {
    ...
    summary += `- **Status:** ${execution.status} ${execution.status === "completed" ? "✓" : execution.status === "failed" ? "✗" : ""}\n`;
    summary += `- **Workflow:** ${workflow.name} (${workflow.id || "unknown"})\n`;
    summary += `- **Version Tag:** ${execution.version_tag || "N/A"}\n`;
    ...
    summary += `- **Completed:** ${execution.completed_at ? formatDate(execution.completed_at) : "Still running"}\n`;
    ...
    if (followUpSuggestions && followUpSuggestions.length > 0) {
      summary += `\n## Follow-Up Suggestions (${followUpSuggestions.length})\n\n`;
      followUpSuggestions.forEach((suggestion, index) => {
        summary += `### ${index + 1}. ${suggestion.name || suggestion.workflowId}\n`;
        summary += `- **Workflow ID:** ${suggestion.workflowId}\n`;
        if (suggestion.description) {
          summary += `- **Description:** ${suggestion.description}\n`;
        }
    ...
    if (stepExecutions.length > 0) {
      ...
      stepExecutions.forEach((step, index) => {
        ...
        summary += `- **Completed:** ${step.completed_at ? formatDate(step.completed_at) : "N/A"}\n`;
