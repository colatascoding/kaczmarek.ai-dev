...
function loadDashboard(dashboardIdOrPath, cwd = process.cwd()) {
  ...
  let item = items.find(i => i.name === dashboardIdOrPath);
  
  // If not found, try to find by path (partial match)
  if (!item) {
    item = items.find(i => i.path.includes(dashboardIdOrPath) || dashboardIdOrPath.includes(i.name));
  }
  
  // If still not found, try to find by full path format (category/subcategory/name)
  if (!item && dashboardIdOrPath.includes("/")) {
    const pathParts = dashboardIdOrPath.split("/");
  ...
  if (!item) {
    console.warn(`Dashboard not found: ${dashboardIdOrPath}. Available dashboards:`, items.map(i => ({ name: i.name, path: i.path })));
    return null;
  }
