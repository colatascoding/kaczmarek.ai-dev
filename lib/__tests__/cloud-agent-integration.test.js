...
describe("Cursor Cloud Agent Integration", () => {
  ...
  describe("Status Polling and Syncing", () => {
    ...
    test("should handle API errors during status polling gracefully", async () => {
      ...
      // Should handle error gracefully - returns cached status instead of throwing
      const result = await agentModule.actions["check-status"](inputs, context);
      
      // The action returns cached status when API fails
      expect(result).toBeDefined();
      expect(result.success).toBe(true);
      expect(result.task.status).toBe("running"); // Uses cached status
  ...
  describe("End-to-End Flow", () => {
    test("should complete full cycle: launch -> poll -> sync -> complete", async () => {
      ...
      https.request = jest.fn((options, callback) => {
        ...
        if (options.path && options.path.includes("/agents") && options.method === "POST") {
      ...
      expect(launchResult.cloudAgentId).toBe("cloud-agent-e2e");
