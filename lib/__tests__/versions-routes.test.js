...
const fs = require("fs");
const { createTempDir, cleanupTempDir, createMockServer, createMockRequest, createMockResponse } = require("./helpers/test-utils");
...
describe("Version Routes", () => {
  ...
  describe("handleGetStageSummary", () => {
    test("should parse URL correctly for /api/versions/:versionTag/:stage/summary", async () => {
      const req = createMockRequest("/api/versions/0-3/plan/summary");
    ...
    test("should return 404 if version not found", async () => {
      const req = createMockRequest("/api/versions/999-999/plan/summary");
      const res = createMockResponse();
    ...
    test("should return 400 for invalid URL format", async () => {
      const req = createMockRequest("/api/versions/invalid");
      const res = createMockResponse();
  ...
  describe("handleGetPlanningAgentStatus", () => {
    test("should return hasAgent: false if no agent queue directory", async () => {
      const req = createMockRequest("/api/versions/0-3/planning-agent-status");
      const res = createMockResponse();
    ...
    test("should extract branch from cloudStatus.target.branchName", async () => {
      const req = createMockRequest("/api/versions/0-13/planning-agent-status");
      const res = createMockResponse();
    ...
    test("should normalize FINISHED status to completed", async () => {
      const req = createMockRequest("/api/versions/0-15/planning-agent-status");
      const res = createMockResponse();
    ...
    test("should return null agentBranch if no branch information available", async () => {
      const req = createMockRequest("/api/versions/0-16/planning-agent-status");
      const res = createMockResponse();
  ...
  describe("handleCreateVersion", () => {
    ...
    test("should create version with skipFolderCreation option", async () => {
      ...
      // Check that stage folders were NOT created (but base version folder exists for metadata)
      const versionPath = path.join(tempDir, "versions", "v0", "0-2");
      expect(fs.existsSync(versionPath)).toBe(true); // Base folder exists for metadata
      
      // But stage folders should NOT exist
      expect(fs.existsSync(path.join(versionPath, "01_plan"))).toBe(false);
      expect(fs.existsSync(path.join(versionPath, "02_implement"))).toBe(false);
      expect(fs.existsSync(path.join(versionPath, "03_test"))).toBe(false);
      expect(fs.existsSync(path.join(versionPath, "04_review"))).toBe(false);
