...
module.exports = {
  ...
  actions: {
    ...
    "run-prompt": async (inputs, context) => {
      ...
      const defaultModel = process.env.CLAUDE_MODEL || "claude-3-5-sonnet-20241022";
      logger.info(`Calling Claude model ${model || defaultModel}...`);
      ...
      if (!result.success) {
        ...
        const message =
          (result.error && (result.error.message || result.error.type)) ||
          (typeof result.error === "string" ? result.error : "Claude API call failed");
        // Treat as a hard failure so the workflow engine marks the step as failed
        throw new Error(`Claude API error (${result.statusCode || "unknown"}): ${message}`);
      }
