...
async function launchAgent({ apiKey, prompt, repository, branch = null, options = {} }) {
  // API expects: prompt is an object (not string), repository field name unknown
  // Try different prompt object formats and repository field names
  
  // Format 1: prompt as { text: "..." } with repo
  let body = {
    prompt: { text: prompt },
    repo: repository,
    ...(branch && { branch })
  ...
  console.log("[Cursor API] Attempting format: prompt={text} + repo");
  let result = await callCursorAPI({
    apiKey,
    method: "POST",
    path: "/agents",
    body
  ...
  // Format 2: prompt as { message: "..." } with repo
  if (!result.success && result.statusCode === 400) {
    console.log("[Cursor API] Retrying format: prompt={message} + repo");
    body = {
      prompt: { message: prompt },
      repo: repository,
      ...(branch && { branch })
    };
    result = await callCursorAPI({
      apiKey,
      method: "POST",
      path: "/agents",
      body
  ...
  // Format 3: prompt as { instruction: "..." } with repo
  if (!result.success && result.statusCode === 400) {
    console.log("[Cursor API] Retrying format: prompt={instruction} + repo");
    body = {
  ...
  // Format 4: prompt as { content: "..." } with repo
  if (!result.success && result.statusCode === 400) {
    console.log("[Cursor API] Retrying format: prompt={content} + repo");
    body = {
  ...
  // Format 5: prompt as array of messages [{ role: "user", content: "..." }]
  if (!result.success && result.statusCode === 400) {
    console.log("[Cursor API] Retrying format: prompt=[{role,content}] + repo");
    body = {
  ...
  // Format 6: prompt as { text: "..." } with repo_url
  if (!result.success && result.statusCode === 400) {
    console.log("[Cursor API] Retrying format: prompt={text} + repo_url");
    body = {
      prompt: { text: prompt },
      repo_url: repository,
      ...(branch && { branch })
  ...
  // Format 7: prompt as { text: "..." } with GitHub URL
  if (!result.success && result.statusCode === 400 && !repository.startsWith("http")) {
    console.log("[Cursor API] Retrying format: prompt={text} + repo (GitHub URL)");
