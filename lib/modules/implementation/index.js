  ...
  if (agentType === "cursor" && process.env.CURSOR_API_KEY) {
    try {
      ...
      const baseGoalSection = highLevelGoal 
        ? `\n\nPRIMARY GOAL FOR THIS VERSION:\n"${highLevelGoal}"\n\nYour task is to analyze this high-level goal and break it down into related, actionable goals organized into 3 workstreams (A, B, C). Each workstream should represent a logical grouping of related tasks that together contribute to achieving the primary goal.`
        : "";
...
4. Organize goals into 3 workstreams (A, B, C) that can be worked on in parallel
