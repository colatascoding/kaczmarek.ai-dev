...
async function launchPlanningAgent(inputs, context) {
  const { 
    ...
    highLevelGoal = null, // High-level goal provided by user
    workstreamCount = 3 // Number of workstreams to create (default: 3)
  } = inputs;
  ...
  if (agentType === "cursor" && process.env.CURSOR_API_KEY) {
    try {
      ...
      const workstreamLabels = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'].slice(0, workstreamCount);
      const workstreamLabelsText = workstreamLabels.join(', ');
      
      const baseGoalSection = highLevelGoal 
        ? `\n\nPRIMARY GOAL FOR THIS VERSION:\n"${highLevelGoal}"\n\nYour task is to analyze this high-level goal and break it down into related, actionable goals organized into ${workstreamCount} workstreams (${workstreamLabelsText}). Each workstream should represent a logical grouping of related tasks that together contribute to achieving the primary goal.`
        : "";

      // Generate workstream examples in the prompt
      const workstreamExamples = workstreamLabels.map((label, index) => {
        return `### Workstream ${label}: [Theme/Feature Area Name]
- [ ] Goal ${label}1 - [Description] (Task 1 - must complete before ${label}2)
- [ ] Goal ${label}2 - [Description] (Task 2 - depends on ${label}1, must complete before ${label}3)
- [ ] Goal ${label}3 - [Description] (Task 3 - depends on ${label}2, optional, max 3 goals per workstream)`;
      }).join('\n\n');

      const planningPrompt = `Generate a comprehensive plan for version ${versionTag} of this project.${baseGoalSection}

Context:
- Current version: ${currentVersionTag || "N/A"}
- New version: ${versionTag}
- Project location: ${cwd}
${highLevelGoal ? `- Primary goal: ${highLevelGoal}` : ""}
- Number of workstreams: ${workstreamCount}
...
3. ${highLevelGoal ? `Break down the primary goal "${highLevelGoal}" into related, actionable sub-goals` : "Generate a detailed plan with primary objectives and goals for this version"}
4. Organize goals into exactly ${workstreamCount} workstreams (${workstreamLabelsText}) that can be worked on in parallel
5. Generate success criteria, key features, and technical considerations
...
${highLevelGoal ? `- Ensure all goals contribute to achieving: "${highLevelGoal}"` : ""}

IMPORTANT: 
- Organize goals into exactly ${workstreamCount} workstreams (${workstreamLabelsText})
- Each workstream should have 1-3 goals (preferably 3)
- Goals within a workstream are SEQUENTIAL - each goal depends on the previous one
- Tasks within a workstream must be completed in order (Task 1 → Task 2 → Task 3)
- Workstreams themselves are PARALLEL - different workstreams can run simultaneously
- Each workstream's goals should be related and build upon each other
- Distribute goals evenly across workstreams when possible
- Each workstream should have a clear theme/feature area name
- When listing goals, indicate dependencies: "Goal X1 (Task 1) → Goal X2 (Task 2, depends on X1) → Goal X3 (Task 3, depends on X2)"
${highLevelGoal ? `- All workstreams should contribute to the primary goal: "${highLevelGoal}"` : ""}`;
...
module.exports = {
  ...
  actions: {
    ...
    /**
     * Automatically create workstreams from goals.md after planning
     */
