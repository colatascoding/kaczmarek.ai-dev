...
async function launchWorkstreamAgent(inputs, context) {
  ...
  // Check workstream mode (parallel or sequential)
  const workstreamMode = metadata.mode || 'sequential'; // Default to sequential for backward compatibility
  ...
  // Declare variables for both parallel and sequential modes
  let nextTask;
  let totalTasks;
  let isLastTask;
  ...
  // For parallel mode, launch all goals at once (if not already launched)
  if (workstreamMode === 'parallel') {
    if (workstreamGoalsOrdered.length === 0) {
      return {
        success: false,
    ...
    // In parallel mode, launch all goals simultaneously
    // For now, we'll launch the first goal (since Cursor Cloud Agents can handle multiple goals in one prompt)
    // TODO: Consider launching multiple agents in parallel if needed
    const parallelPrompt = `Implement workstream: ${workstreamId} for version ${versionTag}

Workstream Description:
${metadata.description || "No description provided"}

**PARALLEL EXECUTION MODE - All goals should be implemented together:**

${workstreamGoalsOrdered.map((g, i) => `${i + 1}. ${g.text}`).join("\n")}

Context:
- Version: ${versionTag}
- Workstream: ${workstreamId}
- Mode: Parallel (all goals executed together)
- Project location: ${cwd}

Task:
1. Implement ALL goals listed above in parallel
...
- Keep review/progress docs in sync`;
    ...
    // Use the first goal as the "next task" but the prompt includes all goals
    nextTask = workstreamGoalsOrdered[0];
    currentTaskIndex = 0;
    totalTasks = workstreamGoalsOrdered.length;
    isLastTask = true; // In parallel mode, there's only one "task" (all goals together)
    remainingTasks = workstreamGoalsOrdered; // All goals for parallel mode
    
    // Store parallel prompt in a local variable (don't mutate metadata)
    implementationPrompt = parallelPrompt;
  } else {
    // For sequential mode, validate and get next task
    ...
    nextTask = workstreamGoalsOrdered[currentTaskIndex];
    totalTasks = metadata.totalTasks || workstreamGoalsOrdered.length;
    isLastTask = currentTaskIndex >= workstreamGoalsOrdered.length - 1;
    remainingTasks = workstreamGoalsOrdered.slice(currentTaskIndex);
  }
  ...
  // remainingTasks is already set in the if/else block above
  ...
  if (agentType === "cursor" && process.env.CURSOR_API_KEY) {
    try {
      ...
      // Build implementation prompt - use parallel prompt if mode is parallel
      if (!implementationPrompt) {
        // Sequential mode - focus on current task
        const currentTaskNum = currentTaskIndex + 1;
        // remainingTasks already set in the else block above
        ...
        implementationPrompt = `Implement workstream: ${workstreamId} for version ${versionTag}
...
- Complete the current task fully before moving to the next`;
      ...
      const agentTask = {
        ...
        autoMerge: workstreamMode === 'sequential' ? true : false, // Auto-merge only for sequential tasks
        mergeStrategy: mergeStrategy,
        isWorkstreamTask: true, // Flag to identify workstream tasks
        launchNextOnComplete: workstreamMode === 'sequential' && !isLastTask // Launch next task only for sequential mode
      };
 ...
 */
async function setWorkstreamGoals(inputs, context) {
  const { cwd = process.cwd(), versionTag, workstreamId, goals: newGoals } = inputs;
  const { logger } = context;

  if (!versionTag || !workstreamId || !newGoals) {
    throw new Error("Version tag, workstream ID, and goals are required");
  }
  ...
  // Normalize and sort new goals
  const normalizedGoals = newGoals.map((goal, index) => {
    if (typeof goal === "string") {
      return { text: goal.trim(), completed: false, taskSequence: index + 1 };
    } else if (typeof goal === "object" && goal !== null) {
      return {
        text: (goal.text || "").trim(),
        completed: goal.completed === true,
        taskSequence: goal.taskSequence !== undefined ? parseInt(goal.taskSequence, 10) : index + 1
      };
    }
    return null;
  }).filter(g => g && g.text);

  normalizedGoals.sort((a, b) => a.taskSequence - b.taskSequence);

  metadata.goals = normalizedGoals;
  metadata.totalTasks = normalizedGoals.length;
  metadata.currentTaskIndex = 0; // Reset task index when goals are set
  metadata.updated = new Date().toISOString().split("T")[0];

  try {
    fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2), "utf8");
    logger.info(`Set goals for workstream: ${workstreamId}`);

    return {
      success: true,
      workstreamId: workstreamId,
      goalsSet: normalizedGoals.length,
      metadata: metadata
    };
  } catch (writeError) {
    logger.error(`Failed to write workstream goals to metadata: ${writeError.message}`);
    throw new Error(`Failed to set workstream goals: ${writeError.message}`);
  }
}
