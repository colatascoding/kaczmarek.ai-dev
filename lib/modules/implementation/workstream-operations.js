...
async function launchWorkstreamAgent(inputs, context) {
  ...
  let workstreamLabel = metadata.workstreamLabel;
  if (!workstreamLabel) {
  ...
  if (!workstreamLabel) {
    return {
      success: false,
      error: `Could not find workstream label for ${workstreamId}. Workstream may not have been created from goals.`
    };
  }
  
  // Get workstream-specific goals, ordered by task sequence
  const workstreamGoals = goalsData.workstreams[workstreamLabel]?.goals || [];
  ...
  // Check current task index in workstream metadata
  let currentTaskIndex = metadata.currentTaskIndex || 0;
  
  // Validate current task index
  if (currentTaskIndex < 0) {
    logger.warn(`Invalid currentTaskIndex ${currentTaskIndex}, resetting to 0`);
    metadata.currentTaskIndex = 0;
    currentTaskIndex = 0;
    try {
      fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2), "utf8");
    } catch (e) {
  ...
  const uncompletedGoals = remainingTasks;
