...
module.exports = {
  ...
  actions: {
    ...
    "wait-for-decision": async (inputs, context) => {
      ...
      // Handle proposals - could be array, JSON string, or single object
      let proposalsArray = [];
      if (Array.isArray(proposals)) {
        ...
        try {
          ...
          proposalsArray = Array.isArray(parsed) ? parsed : [parsed];
        } catch (e) {
          // Not JSON, treat as single proposal
          proposalsArray = [{ id: "default", label: proposals, description: proposals }];
        }
      } else if (proposals) {
        // Single object
        proposalsArray = [proposals];
