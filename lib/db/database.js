...
class WorkflowDatabase {
  ...
  createExecution(id, workflowId, triggerType, triggerData, versionTag = null) {
    const stmt = this.db.prepare(`
      INSERT INTO executions (id, workflow_id, version_tag, trigger_type, trigger_data, status)
      VALUES (?, ?, ?, ?, ?, 'running')
    `);
    stmt.run(id, workflowId, versionTag, triggerType, JSON.stringify(triggerData));
  }
