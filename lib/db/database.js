...
class WorkflowDatabase {
  ...
  runMigrationsSync() {
    try {
      ...
      migrationRunner.initializeMigrationsTable();
  ...
  listExecutions(workflowId = null, status = null, versionTag = null, limit = 100) {
    ...
    return rows.map(row => {
      ...
      try {
        if (row.trigger_data) {
          result.trigger_data = JSON.parse(row.trigger_data);
        } else {
          result.trigger_data = null;
        }
      } catch (e) {
        this.logger.warn(`Failed to parse trigger_data for execution ${row.id}: ${e.message}`);
        result.trigger_data = null;
      }
      
      try {
        if (row.state) {
          result.state = JSON.parse(row.state);
        } else {
          result.state = null;
        }
      } catch (e) {
        this.logger.warn(`Failed to parse state for execution ${row.id}: ${e.message}`);
        result.state = null;
      }
  ...
  getStepExecutions(executionId) {
    ...
    return rows.map(row => {
      const result = { ...row };
      try {
        result.inputs = row.inputs ? JSON.parse(row.inputs) : null;
      } catch (e) {
        this.logger.warn(`Failed to parse inputs for step ${row.id}: ${e.message}`);
        result.inputs = null;
      }
      
      try {
        result.outputs = row.outputs ? JSON.parse(row.outputs) : null;
      } catch (e) {
        this.logger.warn(`Failed to parse outputs for step ${row.id}: ${e.message}`);
        result.outputs = null;
      }
  ...
  getHistory(executionId) {
    ...
    return rows.map(row => {
      const result = { ...row };
      try {
        result.data = row.data ? JSON.parse(row.data) : null;
      } catch (e) {
        this.logger.warn(`Failed to parse history data for execution ${executionId}: ${e.message}`);
        result.data = null;
      }
      return result;
    });
