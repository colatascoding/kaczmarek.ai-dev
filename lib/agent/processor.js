...
class AgentProcessor {
  ...
  async handleAutoMerge(task, taskFilePath) {
    ...
    try {
      ...
      let branch = null;
      if (task.cloudStatus?.source?.ref) {
        branch = task.cloudStatus.source.ref;
      } else if (task.cloudAgentId) {
        // Try to infer branch from agent ID or use a default pattern
        // Cloud agents often work on branches like "agent-{agentId}" or similar
        ...
        try {
          ...
          const branchPatterns = [
            `agent-${task.cloudAgentId.substring(0, 8)}`,
            `cursor-${task.cloudAgentId.substring(0, 8)}`,
            task.cloudAgentId.substring(0, 16)
          ];
          
          for (const pattern of branchPatterns) {
            if (branches.includes(pattern)) {
              branch = pattern.replace(/^origin\//, "").trim();
