  ...
  async pollCloudAgentStatus(task, taskFilePath) {
    ...
    try {
      ...
      if (statusResult.status === "completed") {
        ...
        // If this is a planning agent, auto-create workstreams from goals
        if (task.taskType === "planning" && task.versionTag) {
          try {
            this.log(`Planning agent completed. Auto-creating workstreams from goals...`);
            const workstreamOps = require("../modules/implementation/workstream-operations");
            const result = await workstreamOps.createWorkstreamsFromGoals(
              {
                cwd: this.cwd,
            ...
            if (result.success && result.workstreamsCreated > 0) {
              this.log(`Auto-created ${result.workstreamsCreated} workstream(s) from goals`);
              task.workstreamsCreated = result.workstreams;
              fs.writeFileSync(taskFilePath, JSON.stringify(task, null, 2));
            } else if (result.errors && result.errors.length > 0) {
              this.warn(`Some workstreams failed to create: ${result.errors.join(", ")}`);
            } else {
              this.log(`No workstreams to create (${result.message || "no workstream-organized goals found"})`);
            }
          } catch (error) {
            // Don't fail the task if workstream creation fails
            this.warn(`Failed to auto-create workstreams: ${error.message}`);
          }
        }
