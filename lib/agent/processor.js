...
class AgentProcessor {
  ...
  async processTask(task, taskFilePath) {
    ...
    // Double-check after setting (defensive programming)
    ...
    try {
      ...
      if (statusResult.status === "completed") {
        ...
        if (task.autoMerge === true) {
          try {
            const mergeResult = await this.handleAutoMerge(task, taskFilePath);
            mergeCompleted = mergeResult?.success === true || mergeResult?.merged === true || mergeResult?.alreadyMerged === true;
            if (mergeCompleted) {
              this.log(`Merge completed for workstream task ${task.cloudAgentId}`);
            }
          } catch (mergeError) {
            this.error(`Merge failed for workstream task: ${mergeError.message}`);
            mergeCompleted = false;
          }
        } else {
          // If auto-merge is not enabled but this is a sequential task, we still need to merge
          mergeCompleted = true; // Assume merge is not needed or already done
        }
        
        // Handle workstream sequential tasks
        if (task.isWorkstreamTask && task.workstreamId && task.launchNextOnComplete) {
          try {
            // Update workstream metadata to mark current task as complete
            const versionOps = require("../versions/file-operations");
            const workstreamPath = versionOps.getWorkstreamPath(task.versionTag, task.workstreamId, this.cwd);
            let metadataPath = null;
            if (workstreamPath) {
              metadataPath = path.join(workstreamPath, "workstream.json");
              if (fs.existsSync(metadataPath)) {
                // Use file locking or atomic update to prevent race conditions
                const metadata = JSON.parse(fs.readFileSync(metadataPath, "utf8"));
                const oldIndex = metadata.currentTaskIndex || 0;
                // Only update if index hasn't changed (prevents race conditions)
                if (oldIndex === (task.currentTaskIndex || 0)) {
                  metadata.currentTaskIndex = oldIndex + 1;
                  metadata.lastTaskCompleted = new Date().toISOString();
                  metadata.lastCompletedAgentId = task.cloudAgentId;
                  fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2));
                  this.log(`Updated workstream ${task.workstreamId} task index: ${oldIndex} → ${oldIndex + 1}`);
                } else {
                  this.warn(`Workstream ${task.workstreamId} task index mismatch: expected ${task.currentTaskIndex}, found ${oldIndex}. Skipping update.`);
                }
              }
            }
            
            // Wait for merge to complete before launching next task
            if (mergeCompleted || task.autoMerge !== true) {
              // Small delay to ensure merge is fully processed
              await new Promise(resolve => setTimeout(resolve, 1000));
              
              // Launch next task in sequence
              const nextTaskIndex = (task.currentTaskIndex || 0) + 1;
              this.log(`Launching next task for workstream ${task.workstreamId} (task ${nextTaskIndex + 1} of ${task.totalTasks || "?"})`);
              
              try {
                const implementationModule = require("../modules/implementation");
                // Pass the next task index explicitly to ensure correct task is launched
                const nextTaskResult = await implementationModule.actions["launch-workstream-agent"]({
                  cwd: this.cwd,
                  ...
                  currentTaskIndex: nextTaskIndex // Pass the next task index explicitly
                }, {
                  logger: {
                    info: (msg) => this.log(msg),
                    error: (msg) => this.error(msg),
                ...
                if (nextTaskResult.success) {
                  this.log(`Next task launched for workstream ${task.workstreamId}: ${nextTaskResult.cloudAgentId}`);
                } else {
                  this.warn(`Failed to launch next task for workstream ${task.workstreamId}: ${nextTaskResult.error}`);
                  // Update metadata to indicate failure
                  if (workstreamPath && metadataPath && fs.existsSync(metadataPath)) {
                    try {
                      const metadata = JSON.parse(fs.readFileSync(metadataPath, "utf8"));
                      metadata.lastTaskLaunchError = nextTaskResult.error;
                      metadata.lastTaskLaunchAttempt = new Date().toISOString();
                      fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2));
                    } catch (e) {
                      this.warn(`Failed to update metadata with launch error: ${e.message}`);
                    }
                  }
                }
              } catch (launchError) {
                this.error(`Error launching next task for workstream ${task.workstreamId}: ${launchError.message}`);
                // Update metadata to indicate error
                if (workstreamPath && metadataPath && fs.existsSync(metadataPath)) {
                  try {
                    const metadata = JSON.parse(fs.readFileSync(metadataPath, "utf8"));
                    metadata.lastTaskLaunchError = launchError.message;
                    metadata.lastTaskLaunchAttempt = new Date().toISOString();
                    fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2));
                  } catch (e) {
                    this.warn(`Failed to update metadata with launch error: ${e.message}`);
              ...
              this.warn(`Merge not completed, skipping next task launch for workstream ${task.workstreamId}`);
            }
          } catch (workstreamError) {
            this.error(`Failed to launch next workstream task: ${workstreamError.message}`);
            // Don't fail the task completion if next task launch fails
          }
        }
        
        // Auto-create workstreams from goals.md if this is a planning agent
        if (task.type === "planning" || task.taskType === "planning") {
          try {
            const implementationModule = require("../modules/implementation");
            await implementationModule.actions["create-workstreams-from-goals"]({
              cwd: this.cwd,
              versionTag: task.versionTag
            }, { 
              logger: {
            ...
            this.log(`Auto-created workstreams from goals for version ${task.versionTag}`);
          } catch (workstreamError) {
            this.error(`Failed to auto-create workstreams: ${workstreamError.message}`);
            // Don't fail the task completion if workstream creation fails
          }
        }
  ...
  async handleAutoMerge(task, taskFilePath) {
    ...
    try {
      ...
      return {
        ...
        merged: mergeResult.merged || mergeResult.alreadyMerged || false,
