...
class AgentProcessor {
  ...
  async processWithCursor(task, taskFilePath) {
    ...
    const executor = new AgentExecutor({ 
      cwd: this.cwd,
      logger: {
        info: (msg) => console.log(`[Executor] ${msg}`),
        error: (msg) => console.error(`[Executor] ${msg}`),
        warn: (msg) => console.warn(`[Executor] ${msg}`)
      }
    });
    ...
    try {
      ...
      console.error(`[Agent Processor] Execution error: ${error.message}`);
    ...
    console.log(`[Agent Processor] Use Cursor Chat to process remaining tasks, or mark as complete with 'kad agent complete ${task.id}'`);
