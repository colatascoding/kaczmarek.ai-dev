...
function createLibraryRoutes(server) {
  return {
    ...
    /**
     * List library workflows (for library view)
     */
    async handleListLibraryWorkflows(req, res) {
      // Use workflow discovery to get all workflows, including library ones
      const workflows = workflowDiscovery.discoverWorkflows(server.cwd, {});
      
      // Filter to only library workflows
      const libraryWorkflows = workflows.filter(wf => wf.source === "library");
      
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ workflows: libraryWorkflows }));
    },
    ...
    async handleRenderDashboard(req, res) {
      // Parse URL to get dashboard ID (before /render)
      const url = new URL(req.url, `http://${req.headers.host}`);
      const pathParts = url.pathname.split("/").filter(p => p);
      
      // Find the index of "render" and get the part before it
      const renderIndex = pathParts.indexOf("render");
      if (renderIndex === -1 || renderIndex === 0) {
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Invalid dashboard render URL" }));
        return;
      }
      
      // Get dashboard ID (could be just name or full path like "version-overview/version-status")
      const dashboardIdParts = pathParts.slice(pathParts.indexOf("dashboards") + 1, renderIndex);
      const dashboardId = dashboardIdParts.join("/");
