  ...
  return {
    ...
    async handleGetPlanningAgentStatus(req, res) {
      try {
        ...
        let agentBranch = task.agentBranch || 
                         task.cloudStatus?.branch || 
                         task.cloudStatus?.ref || 
                         task.cloudStatus?.source?.ref || 
                         null;
        ...
        let agentStatus = {
          ...
          agentBranch: agentBranch // Include branch name for manual merge
        };
        ...
        if (task.type === "cursor-cloud" && task.cloudAgentId && process.env.CURSOR_API_KEY) {
          ...
          if (task.status === "completed" || task.status === "failed") {
            ...
            try {
              ...
              if (statusResult.data?.branch || statusResult.data?.ref || statusResult.data?.source?.ref) {
                agentStatus.agentBranch = statusResult.data.branch || statusResult.data.ref || statusResult.data.source.ref;
                task.agentBranch = agentStatus.agentBranch;
              }
            } catch (error) {
