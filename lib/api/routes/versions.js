  ...
  return {
    ...
    async handleGetPlanningAgentStatus(req, res) {
      try {
        ...
        // Debug: Log task structure to verify it's being read correctly
        console.log(`[Planning Agent Status] Task loaded for ${versionTag}:`, {
          id: task.id,
          versionTag: task.versionTag,
          hasCloudStatus: !!task.cloudStatus,
          cloudStatusKeys: task.cloudStatus ? Object.keys(task.cloudStatus) : [],
          hasTarget: !!task.cloudStatus?.target,
          targetKeys: task.cloudStatus?.target ? Object.keys(task.cloudStatus.target) : [],
          targetBranchName: task.cloudStatus?.target?.branchName,
        ...
        // Debug logging
        console.log(`[Planning Agent Status] Branch extraction for ${versionTag}:`, {
          "task.agentBranch": task.agentBranch,
          "task.cloudStatus?.target?.branchName": task.cloudStatus?.target?.branchName,
          "task.cloudStatus?.branchName": task.cloudStatus?.branchName,
          "task.cloudStatus?.source?.ref": task.cloudStatus?.source?.ref,
          "extracted agentBranch": agentBranch
        });
        ...
        console.log(`[Planning Agent Status] Initial extraction for ${versionTag}:`, {
          "agentBranch extracted": agentBranch,
          "task.cloudStatus exists": !!task.cloudStatus,
          "task.cloudStatus.target exists": !!task.cloudStatus?.target,
          "task.cloudStatus.target.branchName": task.cloudStatus?.target?.branchName
        });
        ...
        if (task.cloudStatus?.target?.branchName) {
          ...
          console.log(`[Planning Agent Status] Set agentBranch from target.branchName: ${agentStatus.agentBranch}`);
        } else if (!agentStatus.agentBranch && task.cloudStatus?.branchName) {
          agentStatus.agentBranch = task.cloudStatus.branchName;
          console.log(`[Planning Agent Status] Set agentBranch from branchName: ${agentStatus.agentBranch}`);
        } else if (!agentStatus.agentBranch) {
          console.log(`[Planning Agent Status] WARNING: No branch found for ${versionTag}. cloudStatus:`, {
            hasCloudStatus: !!task.cloudStatus,
            hasTarget: !!task.cloudStatus?.target,
            targetKeys: task.cloudStatus?.target ? Object.keys(task.cloudStatus.target) : [],
            branchName: task.cloudStatus?.target?.branchName
          });
        }
        ...
        console.log(`[Planning Agent Status] Final response for ${versionTag}:`, {
          status: agentStatus.status,
          agentBranch: agentStatus.agentBranch,
          agentBranchType: typeof agentStatus.agentBranch,
          agentBranchIsNull: agentStatus.agentBranch === null,
          agentBranchIsUndefined: agentStatus.agentBranch === undefined,
          autoMerge: agentStatus.autoMerge,
          hasCloudStatus: !!task.cloudStatus,
          cloudStatusTarget: task.cloudStatus?.target,
          taskAgentBranch: task.agentBranch,
          taskCloudStatusTargetBranchName: task.cloudStatus?.target?.branchName
        });
        ...
        if (!("agentBranch" in agentStatus)) {
          ...
          console.log(`[Planning Agent Status] WARNING: agentBranch was missing from agentStatus, set to null`);
        }
        ...
        console.log(`[Planning Agent Status] Sending response for ${versionTag}:`, JSON.stringify(response, null, 2));
          ...
          if (!metadata) {
            ...
            if (!metadata) {
              ...
              if (skipFolderCreation) {
                ...
                const versionMatch = versionTag.match(/^(\d+)-(\d+)$/);
                if (versionMatch) {
