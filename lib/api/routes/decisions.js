...
function createDecisionRoutes(server) {
  return {
    ...
    async handleGetPendingDecisions(req, res, pathname) {
      ...
      // Check if executionId is missing or if it's actually "executions" (meaning empty executionId in path)
      if (!executionId || executionId === "executions" || executionId === "decisions") {
