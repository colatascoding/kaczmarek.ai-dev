...
class APIServer {
  ...
  async handleRequest(req, res) {
    ...
    try {
      if (pathname === "/api/workflows" && method === "GET") {
        ...
        await this.handleNextStep(req, res, pathname);
  ...
  async handleRunWorkflow(req, res, pathname) {
    ...
    req.on("end", async () => {
      try {
        ...
        this.engine.execute(workflowId, params, { executionMode }).then(result => {
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify({
            success: true,
    ...
    try {
      ...
      if (needsRecalculation && execution.status === "completed" && workflow && execution.state) {
        try {
          ...
          const workflowFilePath = path.join(this.workflowsDir, `${workflowId}.yaml`);
          ...
          if (forceRecalculate || !outcome) {
            outcome = this.engine.determineOutcome(state, workflowDef);
          ...
          if (forceRecalculate || !followUpSuggestions || (Array.isArray(followUpSuggestions) && followUpSuggestions.length === 0)) {
            const determinedOutcome = outcome || this.engine.determineOutcome(state, workflowDef);
            followUpSuggestions = this.engine.getFollowUpSuggestions(determinedOutcome, workflowDef);
            ...
            console.log(`[API Server] Workflow has followUpWorkflows: ${!!workflowDef.followUpWorkflows}, count: ${workflowDef.followUpWorkflows?.length || 0}`);
            console.log(`[API Server] Suggestions:`, JSON.stringify(followUpSuggestions, null, 2));
          }
  ...
  async handleNextStep(req, res, pathname) {
    try {
      ...
      const workflowId = execution.workflow_id;
      const workflowDef = this.db.getWorkflow(workflowId);
      ...
      const result = await this.engine.executeNextStep(executionId, workflow, triggerData, execution.version_tag);
      ...
      const files = fs.readdirSync(queueDir)
        ...
        .map(f => {
          ...
          try {
            ...
            if (task.executionId) {
              const execution = this.db.getExecution(task.executionId);
              if (execution) {
                const workflow = execution.workflow_id ? this.db.getWorkflow(execution.workflow_id) : null;
                if (workflow) {
